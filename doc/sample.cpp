...
//  The author of this sketch and Arduino Library does not know the seller of this kit and does not have a commercial relationship with any commercial product that uses the Arduino
//  Library. It is important you understand that there is no guarantee that this sketch will work correctly in your current product. SO, DO NOT TRY IT IF YOU DON'T KNOW WHAT ARE
//  YOU DOING. YOU MUST BE ABLE TO GO BACK TO THE PREVIOUS VERSION IF THIS SKETCH DOES NOT WORK FOR YOU.
...
//  |   T_CLK    |     18      | 10 |            |            |            |        Base with 2K to pin 14 (PIN_DISPLAY_LED)
...
#define R_ATTENNUATOR ((100.0f + 200.0f) / 100.0f)  // A feszültségosztó ellenállások értéke
...
#define VOLTAGE_ATT_CORR -0.02  // A feszültségosztón mért valós feszültség korrigálásához
    ...
    pinMode(29, INPUT);
    float voltageOut = (analogRead(29) * V_REFERENCE) / 65535;
...
void setup() {
    ...
    digitalWrite(PIN_DISPLAY_LED, HIGH);

    // I2C for SI4735
    Wire.setSDA(PIN_SI4735_I2C_SDA);
    Wire.setSCL(PIN_SI4735_I2C_SCL);
    Wire.begin();

    // ledcSetup(LedChannelforTFT, LedFreq, LedResol);
    // ledcAttachPin(PIN_DISPLAY_LED, LedChannelforTFT);
    ...
    tft.setCursor(0, 0);
    tft.println("SI4735 RADIO_V5.2b            Dark Mode");

    spr.createSprite(265, 120);
    spr.fillScreen(COLOR_BACKGROUND);
    spr.pushImage(0, 0, 265, 120, (uint16_t *)logo);
    spr.pushSprite(27, 35);
    spr.deleteSprite();

    // tft.println("SI4735 Radio_Version 5.2b_DarkMode");
    //   tft.setCursor(7, 70);
    //   tft.println(" Version 5.2b Dark");
    //   tft.setCursor(7, 95);
    //   tft.println(" 17-09-2022");
    //   tft.setTextColor(TFT_WHITE, TFT_BLACK);
    //   tft.setCursor(7, 120);
    //   tft.println(" V.4.0 Binns MOD By IU4ALH");
    ...
    if (EEPROM.read(offsetEEPROM) != storage.chkDigit or digitalRead(PIN_ENCODER_SW) == LOW) {
        beeper.error();
    ...
    ITimer1.attachInterruptInterval(TIMER1_INTERVAL_USEC, hardwareTimerHandler1_ForRotary);

    si4735.setAudioMuteMcuPin(PIN_AUDIO_MUTE);
    ...
    for (int i = 0; i <= lastMemoBank; i++) {
        if (i > lastMemoBankFile) {
            ...
            int n = 0;
            int j = 0;
            while (MemoBankFile[i].Name[j] != '\0' and n < 21) {
...
#ifdef IhaveCrystal
    if (bandIdx == 0)
        si4735.setup(PIN_SI4735_RESET, FM_BAND_TYPE);  // Start in FM
    else
        si4735.setup(PIN_SI4735_RESET, 1);
    ...
    if (bandIdx == 0)
        si4735.setup(PIN_SI4753_RESET, -1, POWER_UP_FM, SI473X_ANALOG_AUDIO, XOSCEN_RCLK);  // Start in FM
    else
        si4735.setup(PIN_SI4753_RESET, -1, POWER_UP_AM, SI473X_ANALOG_AUDIO, XOSCEN_RCLK);  // Start in AM
    ...
    // Ráerőszakoljuk a beállításokat (a konfig betöltéssel még baj van)
    RDSalways = true;  // RDS
    ///////////batShow = true;   // Batterry
...
void loadMemo() {
    ...
    if (EEPROM.read(offsetEEPROM + 0) == storage.chkDigit) {
        for (unsigned int t = 0; t < sizeof(MemoBank); t++) *((char *)&MemoBank + t) = EEPROM.read(offsetMemoEEPROM + t);
...
void loadConfig() {
    //=======================================================================================
    if (EEPROM.read(offsetEEPROM + 0) == storage.chkDigit) {
        for (unsigned int t = 0; t < sizeof(storage); t++) *((char *)&storage + t) = EEPROM.read(offsetEEPROM + t);
        Serial.println("Load config done");
    }
}
...
void printConfig() {
    ...
    if (EEPROM.read(offsetEEPROM) == storage.chkDigit) {
        for (unsigned int t = 0; t < sizeof(storage); t++) Serial.write(EEPROM.read(offsetEEPROM + t));
        Serial.println();
    }
}
...
void Freqcalq(int keyval) {
    ...
    if ((realFreq > 0 and realFreq < 31) or (realFreq >= 64 and realFreq <= 108) or (realFreq > 152 and realFreq <= 30000))
        ...
        if (Decipoint) {
            ...
            if (realFreq < 153) {
                ...
                if (realFreq < 1000)
                    ...
                    sprintf(s, "%i %03i", khz, hz);
            }
            tft.drawString(s, 10, 114);
        }
    } else
        tft.print("0");
    if (!Decipoint and DisplayfreqNew >= (band[0].minimumFreq / 10) and DisplayfreqNew <= (band[0].maximumFreq / 10))
        drawButton(L_FREQ, 9, B_NORMAL);
    else
        drawButton(L_FREQ, 9, B_BLOCK);
    tft.setTextSize(2);
...
void Smeter() {
    ...
    while (met > 11 and tik < 9) {
        if (tik)
            tft.fillRect(Xsmtr + 20 + (tik * 12), Ysmtr + 38, 10, 6, TFT_ORANGE);
        else
            tft.fillRect(Xsmtr + 15, Ysmtr + 38, 15, 6, TFT_RED);
        met -= 12;
        tik++;
    }
...
void saver() {
    ...
    if (displayOff) {
        if (displayPower)
            digitalWrite(PIN_DISPLAY_LED, 1);
        else {
    ...
    if (saverOn) {
        ...
        if (SCANbut)
            freq = currentScanFreq + int((currentScanLine - 159 + deltaScanLine) * SCANstep);
        else if (currentMode == LSB || currentMode == USB || currentMode == CW)
            freq -= int(currentBFO / 1000);
    }

    // WAIT activity
    while (((tftPressed == false) and (rotaryEncoderResult.direction == RotaryEncoder::Direction::NONE) and (rotaryEncoderResult.buttonState == RotaryEncoder::Open) and
            (digitalRead(PIN_ENCODER_SW) == HIGH)) or
           (writingEeprom)) {  // wait loop
        rotaryEncoderResult = rotaryEncoder.readRotaryEncoder();
        tftPressed = tft.getTouch(&x, &y);
...
void returnLayer() {
    ...
    if (BandWidth) {
        if (currentMode == AM)
            drawList(L_BANDW_AM, "AM Filter in KHz");
        else if (currentMode == FM)
            drawList(L_BANDW_FM, "FM Filter in KHz");
        else
            drawList(L_BANDW_SSB, "SSB Filter in KHz");
    }
    if (STEPbut) {
        if (currentMode == AM)
            drawList(L_STEP_AM, "STEP TUNE AM");
        else
            drawList(L_STEP_FM, "STEP TUNE FM");
    }
...
void loop() {
    ...
    while (((tftPressed == false) and (rotaryEncoderResult.direction == RotaryEncoder::Direction::NONE) and (rotaryEncoderResult.buttonState == RotaryEncoder::Open) and
        ...
        if (scrollRetro) {
            ...
            if (scrollRetro) {
                ...
                if ((scrollRetro == 1 and (currentRetroFreq + band[bandIdx].currentStep) <= tmpmax) or
                    (scrollRetro != 1 and (currentRetroFreq - band[bandIdx].currentStep) >= tmpmin)) {
                    if (scrollRetro == 1)
                        si4735.frequencyUp();
                    else
            ...
            if (RETROband < 2)
                bandRetro[RETROband].currentFreq = currentRetroFreq / 100;
            else
                bandRetro[RETROband].currentFreq = currentRetroFreq;
            drawRETROscale();
        }
        if (PRESbut) {
        ...
        if (FirstLayer) {  //==================================================
            ...
            if ((currentMode == LSB || currentMode == USB || currentMode == CW) and (x > 139) and (x < 219) and (y > 25) and (y < 60)) {
                for (int n = 0; n <= lastdignum; n++) {
                    if ((x > (dn[n].Xdignumos) + (dn[n].Xdignumnr)) and (x < ((dn[n].Xdignumos) + (dn[n].Xdignumsr) + (dn[n].Xdignumnr))) and
                        (y > (dn[n].Ydignumos) and (y < ((dn[n].Ydignumos) + (dn[n].Ydignumsr))))) {
                        beeper.tick();
                        delay(200);
                        if (bfoOn == false) {
                            freqstepnr = n;
            ...
            if (n >= 0) {
                ...
                if (n == B_ATT) {
                    ...
                    FreqDispl();
                    if (AGCgainbut)
                        drawButton(L_FIRST, B_ATT, B_SELECT);
                    else
                        drawButton(L_FIRST, B_ATT, B_NORMAL);
                ...
                if (n == B_VOL) {  //================================== VOL button
                    ...
                    FreqDispl();
                    if (VOLbut)
                        drawButton(L_FIRST, B_VOL, B_SELECT);
                    else
                        drawButton(L_FIRST, B_VOL, B_NORMAL);
                }
                ...
                for (int n = 0; n <= lastBandRetro; n++) {
                    if ((screenV and x > bandRetro[n].xPosV and x < (bandRetro[n].xPosV + But_Width) and y > bandRetro[n].yPosV and y < (bandRetro[n].yPosV + But_Height)) or
                        ...
                        if (VHFon or n != 1) {
                            ...
                            beeper.error();
                            Serial.println("Band retro button");
                        }
                    }
                }
            }
                        ...
                        if ((int(y / 40) - 3 + currentMemo) >= 0 and (int(y / 40) - 3 + currentMemo) <= lastMemoBank) {
                            ...
                            if (presetBank) {
                                ...
                                if (tmpBand == 29 and (preset[int(y / 40) - 3 + currentMemo].presetIdx < 153 or preset[int(y / 40) - 3 + currentMemo].presetIdx > 30000)) {
                                    ...
                                    if (tmpBand)
                                        si4735.setFrequency(preset[int(y / 40) - 3 + currentMemo].presetIdx);
                                    else
                                        si4735.setFrequency(preset[int(y / 40) - 3 + currentMemo].presetIdx * 100);
                                    band[bandIdx].currentFreq = si4735.getFrequency();
                                    FreqDispl();
                                }
                            } else {
                                if (MemoBank[int(y / 40) - 3 + currentMemo].freq < 153 or MemoBank[int(y / 40) - 3 + currentMemo].freq > 30000) {
                                    beeper.error();
                                    Serial.println("Memo button 2");
                                } else {
                                    beeper.tick();
                                    if (bandIdx != (MemoBank[int(y / 40) - 3 + currentMemo].band & 0x1F) or
            ...
            if (SCANbut and STEPbut == false) {
                ...
                if (n >= 0) {
                    if (n == 0) {  // SCALE
                        ...
                        if (SCANstep == currentMinScanStep)
                    ...
                    if (n >= 0) {
                        if (n != 9 or (!Decipoint and DisplayfreqNew >= (band[0].minimumFreq / 10) and DisplayfreqNew <= (band[0].maximumFreq / 10))) {
                            ...
                            if (n == 11) {  // SET button
                                ...
                                if ((DisplayfreqNew > 30 and DisplayfreqNew < (band[0].minimumFreq / 100)) or (DisplayfreqNew > 108 and DisplayfreqNew < 153) or
                                    DisplayfreqNew == 0) {
                                    tft.setTextSize(2);
                                    ...
                                    beeper.error();
                                    Serial.println("Freq not support");
                                } else {
                                    if ((DisplayfreqNew >= (band[0].minimumFreq / 100)) and (DisplayfreqNew <= (band[0].maximumFreq / 100))) {
                                        ...
                                        // LW/MW/SW
                                        if (DisplayfreqNew < 153)
                                            currentFrequency = DisplayfreqNew * 1000;
                                        else
                                            currentFrequency = DisplayfreqNew;
        ...
        if (ThirdLayer) {  //==================================================
            ...
            if (n >= 0) {
                ...
                if (bfoOn and (n == B_SCAN or n == B_RETRO or n == B_FM or n == B_MEMO or n == B_SETUP)) {
                    bfoOn = false;
                    if (n == B_FM)
                        drawButton(L_FIRST, B_BFO, B_BLOCK);
                    else
                        drawButton(L_FIRST, B_BFO, B_NORMAL);
                    DrawDispl();
                }
    ...
    if (currentAGCgain != previousAGCgain) {
        ...
        if (si4735.isCurrentTuneFM())
            MaxAGCgain = MaxAGCgainFM;
        else
            MaxAGCgain = MaxAGCgainAM;
...
void DisplayRDS() {
    ...
    if ((currentMode == FM) and ((FirstLayer) or (ThirdLayer))) {
        ...
        if ((RDS) and (NewSNR >= 9) and !VOLbut and !AGCgainbut) {
            ...
            tft.fillRect(XFreqDispl + 70, YFreqDispl + 57, 115, 16, TFT_BLACK);  // clear RDS text
        }
    }
}
...
void showRSSI() {
    //=======================================================================================
    // Monó Sztereó megjelenítése
    if ((currentMode == FM) and ((FirstLayer) or (ThirdLayer) or (SecondLayer and RETRObut and !RETROband))) {
        if (RETRObut) {
            int d = screenV * 80;
            if (si4735.getCurrentPilot())
                tft.fillRect(298 - d, 25, 14, 5, TFT_RED);
            else
                tft.fillRect(298 - d, 25, 14, 5, TFT_BLACK);
        } else {
            uint16_t backGroundColor = si4735.getCurrentPilot() ? TFT_RED : TFT_BLUE;
            sprintf(buffer, "%s", (si4735.getCurrentPilot()) ? "STEREO" : "MONO");
            tft.setTextColor(TFT_WHITE, backGroundColor);
            tft.setTextSize(1);
            tft.setTextDatum(BC_DATUM);
            tft.setTextPadding(0);
            tft.fillRect(XFreqDispl + 191, YFreqDispl + 60, 38, 11, backGroundColor);  // STEREO MONO
            tft.drawString(buffer, XFreqDispl + 210, YFreqDispl + 72);
        }
    }
    rssi = NewRSSI;
    if ((FirstLayer) or (ThirdLayer)) Smeter();
    if ((currentMode == AM) or (currentMode == LSB) or (currentMode == USB) or
        (currentMode == CW) and ((FirstLayer) or (ThirdLayer) or (SecondLayer and RETRObut and !RETROband))) {
        tft.setTextSize(1);
        tft.setTextColor(TFT_GREEN, TFT_BLACK);
...
void encoderCheck() {
    ...
    if (rotaryEncoderResult.direction != RotaryEncoder::Direction::NONE) {
        ...
        if (SCANbut and VOLbut == false and bfoOn == false) {
            ...
            float tmpcurline = (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP) ? (currentScanLine + (float(band[bandIdx].currentStep) / SCANstep))
                                                                                               : (currentScanLine - (float(band[bandIdx].currentStep) / SCANstep));
            if ((rotaryEncoderResult.direction == RotaryEncoder::Direction::UP and
                 (currentScanFreq + ((tmpcurline - 159 + d + deltaScanLine) * SCANstep)) <= band[bandIdx].maximumFreq) or
                (rotaryEncoderResult.direction != RotaryEncoder::Direction::UP and
                 (currentScanFreq + ((tmpcurline - 159 + d + deltaScanLine) * SCANstep)) >= band[bandIdx].minimumFreq)) {
                int tmpline = currentScanLine;
                ...
                if (trunc(currentScanLine) < 0) {
                    ...
                    if (trunc(currentScanLine) > (319 - (d * 2))) {
                        ...
                        if (rotaryEncoderResult.direction != RotaryEncoder::Direction::UP) currentScanLine -= 10;
...
#ifdef IhaveCrystal
            if (RETRObut) {
                currentBFOmanu = (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP) ? (currentBFOmanu - currentBFOStep) : (currentBFOmanu + currentBFOStep);
        ...
        if (RETRObut and !VOLbut and !bfoOn) {  // retro scale
            if (scrollRetro) {
                ...
                if ((rotaryEncoderResult.direction == RotaryEncoder::Direction::UP and (currentRetroFreq + band[bandIdx].currentStep) <= tmpmax) or
                    (rotaryEncoderResult.direction != RotaryEncoder::Direction::UP and (currentRetroFreq - band[bandIdx].currentStep) >= tmpmin)) {
                    if (bandHamRetro) {
                        currentRetroFreq++;
                    } else {
                        if (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP)
        ...
        if (PRESbut and !bright) {  // FM preset
            currentPRES = (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP) ? (currentPRES + currentPRESStep) : (currentPRES - currentPRESStep);
        ...
        if (bright) {  // Brightness control
            currentBrightness = (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP) ? (currentBrightness - 10) : (currentBrightness + 10);
            mainpurp = false;
        }
        // ======================== Change the Squelch Setting ========================= LWH
        if (SQUELCHbut) {  // Squelch control
            currentSquelch = (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP) ? (currentSquelch + currentSQUELCHStep) : (currentSquelch - currentSQUELCHStep);
        ...
        if (VOLbut) {  // Volume control
            currentVOL = (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP) ? (currentVOL + currentVOLStep) : (currentVOL - currentVOLStep);
            VOLbutOnTime = millis();
            mainpurp = false;
        }

        if (AGCgainbut) {  // AGC gain control
            currentAGCgain = (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP) ? (currentAGCgain + currentAGCgainStep) : (currentAGCgain - currentAGCgainStep);
            mainpurp = false;
        }

        if (SETUPbut) {
            pageSetup = (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP) ? (pageSetup + 1) : (pageSetup - 1);
        ...
        if (MEMObut) {
            if (MEMOadd) {
                charMemoName = (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP) ? (charMemoName + 1) : (charMemoName - 1);
                if (charMemoName == 144 and rotaryEncoderResult.direction == RotaryEncoder::Direction::UP) charMemoName = 32;
                if (charMemoName == 128 and rotaryEncoderResult.direction == RotaryEncoder::Direction::UP) charMemoName = 144;
                if (charMemoName == 192) charMemoName = 128;
                if (charMemoName == 127 and rotaryEncoderResult.direction != RotaryEncoder::Direction::UP) charMemoName = 191;
                if (charMemoName == 143 and rotaryEncoderResult.direction != RotaryEncoder::Direction::UP) charMemoName = 127;
                if (charMemoName == 31) charMemoName = 143;
            } else {
                currentMemo = (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP) ? (currentMemo + 1) : (currentMemo - 1);
        ...
        if (!PRESbut and !RETRObut and !VOLbut and !AGCgainbut and !bright and !cityRETRObut and !bandRETRObut and !SETUPbut and !MEMObut and !SQUELCHbut)
            FreqDispl();  // LWH - added "and SQUELCHbut"
        if (SCANbut) {
            DrawSCANind();
            DisplaySCANsignal();
        }
...
void encoderButtonCheck() {
    //=======================================================================================
    // Encoder button
    if (digitalRead(PIN_ENCODER_SW) == LOW and (ThirdLayer or FirstLayer or SecondLayer)) {
        beeper.tick();
        long encTime = millis() + 1000;
        while (digitalRead(PIN_ENCODER_SW) == LOW and encTime > millis());
        if (digitalRead(PIN_ENCODER_SW) == LOW) {
            ...
            while (digitalRead(PIN_ENCODER_SW) == HIGH and encTime > millis());
            if (digitalRead(PIN_ENCODER_SW) == LOW) {
                    ...
                    if (MEMOadd) {
                        ...
                        if (presetBank) {
                            ...
                            if (tmpBand == 29 and (preset[currentMemo].presetIdx < 153 or preset[currentMemo].presetIdx > 30000)) {
                                beeper.error();
                                Serial.println("memo add button");
                            } else {
                                beeper.tick();
                                if (bandIdx != tmpBand or currentMode != bandMode[tmpBand]) {
                                    bandIdx = tmpBand;
                                ...
                                if (tmpBand)
                                    si4735.setFrequency(preset[currentMemo].presetIdx);
                                else
                                    si4735.setFrequency(preset[currentMemo].presetIdx * 100);
                                band[bandIdx].currentFreq = si4735.getFrequency();
                                FreqDispl();
                                displMEMO();
                            }
                            ...
                            if (MemoBank[currentMemo].freq < 153 or MemoBank[currentMemo].freq > 30000) {
                                beeper.error();
                                Serial.println("presetBank else");
                            } else {
...
void DrawVolumeIndicator() {
    //=======================================================================================
    tft.setTextSize(1);
    tft.fillRect(XVolInd + 2, YVolInd - 1, 157, 28, TFT_BLACK);
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
...
void DrawBatteryIndicator() {
    //=======================================================================================
    tft.fillRect(XVolInd + 161, YVolInd - 1, 77, 28, TFT_BLACK);
...
void SEEKdispl(int dir) {
    ...
    if (dir)
        tft.drawString("<<<<", XFreqDispl, YFreqDispl + 60);
    else
        tft.drawString(">>>>", XFreqDispl, YFreqDispl + 60);
}
...
void drawListBut(uint8_t lay) {
    ...
    if (BroadBand || HamBand) {
        ...
        if (currentMode == AM)
            drawButton(lay, bwIdxAM, B_JAM);
        else if (currentMode == FM)
            drawButton(lay, bwIdxFM, B_JAM);
        else
            drawButton(lay, bwIdxSSB, B_JAM);
    } else if (STEPbut) {
        if (band[bandIdx].bandType == MW_BAND_TYPE or band[bandIdx].bandType == LW_BAND_TYPE)
            drawButton(lay, ssIdxMW, B_JAM);
        else if (currentMode == FM)
            drawButton(lay, ssIdxFM, B_JAM);
        else
            drawButton(lay, ssIdxAM, B_JAM);
    } else if (Modebut)
        drawButton(lay, currentMode, B_JAM);
}
...
void subrstatus() {
    ...
    tft.setCursor(0, 0);
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    tft.drawString("Software version V5.2b_Dark  17-09-2022 IU4ALH MOD", 5, 0);
    tft.drawString("Mode     : " + String(bandModeDesc[band[bandIdx].prefmod]), 5, 10);
    if (currentMode != FM)
        tft.drawString("Freq.    : " + String(currentFrequency, 0) + " KHz", 5, 20);
    else
        tft.drawString("Freq.    : " + String(currentFrequency / 100, 1) + " MHz", 5, 20);
    si4735.getCurrentReceivedSignalQuality();
    ...
    if (bfoOn)
        tft.drawString("BFO ON  ", 5, 90);
    else
        tft.drawString("BFO OFF ", 5, 90);
    ...
    float vSupply;
    if (batShow) {
        // vsupply = 3.724 * analogRead(PIN_BAT_INFO) / 2047;
        vSupply = readBatterry();
    } else {
        // Az encoder button láb feszültségét mérjük!!!
        vSupply = ((1.66 / 1850) * analogRead(PIN_ENCODER_SW)) * 2;
    }
...
void checkRDS() {
    ...
    if (si4735.getRdsReceived()) {
        if (si4735.getRdsSync() and si4735.getRdsSyncFound()) {
...
// Define to calculate TFT color value from R, G, B values
#define TFT_COLOR(R, G, B) (((R & 0xF8) << 8) | ((G & 0xFC) << 3) | (B >> 3))
                spr.setTextColor(TFT_COLOR(40, 40, 40));  // Nem aktív szegmens színe
            }
        ...
        if (RETRObut) {
            if (screenV) {
                if (RETROband)
                    spr.pushSprite(15, 200);
                else
                    spr.pushSprite(50, 200);
            } else
                spr.pushSprite(125, -3);
        } else
            spr.pushSprite(XFreqDispl + d, YFreqDispl + 20);
    }
...
void FreqDispl() {
    ...
    if (FirstLayer or ThirdLayer or SecondLayer) {
        ...
        if ((VOLbut) or (AGCgainbut) or (SQUELCHbut)) {  // LWH - Added SQUELCHbut
            ...
            if (volDisp)
                tft.fillRect(XFreqDispl + d + 40, YFreqDispl + 20, 55, y, TFT_BLACK);
            else
                tft.fillRect(XFreqDispl + d, YFreqDispl + 20, 240, y, TFT_BLACK);
            if (squelchDisp)
                tft.fillRect(XFreqDispl + d + 40, YFreqDispl + 20, 55, y, TFT_BLACK);
            else
                tft.fillRect(XFreqDispl + d, YFreqDispl + 20, 240, y, TFT_BLACK);
...
void FreqDraw(float freq, int d) {
    ...
    // ELőző érték törlése
    tft.fillRect(XFreqDispl + 46 + d, YFreqDispl + 20, 194, 40, TFT_BLACK);
...
bool checkStopSeeking() {
    ...
    return rotaryEncoderResult.direction != RotaryEncoder::Direction::NONE or tft.getTouch(&x, &y) or
           digitalRead(PIN_ENCODER_SW) == LOW;  // returns true if the user rotates the encoder or touches on screen
}
...
void DrawDispl() {
    ...
    if (BWtext == "AUTO") {
        ...
        tft.drawString("F" + BWtext + "KHz", XFreqDispl + 135, YFreqDispl + 15);
    ...
    if (currentMode == FM) {
        ...
        tft.drawString(String(band[bandIdx].currentStep) + "KHz", XFreqDispl + 220, YFreqDispl + 15);
...
void AGCfreqdisp() {
    ...
    if (AGCgain > 1) {
        if (currentAGCgain > 9)
            tft.drawString("ATT" + String(currentAGCgain), XFreqDispl + 60, YFreqDispl + 15);
        else
            tft.drawString("ATT " + String(currentAGCgain), XFreqDispl + 60, YFreqDispl + 15);
    } else
        tft.drawString(" AGC ", XFreqDispl + 60, YFreqDispl + 15);
...
void DisplaySCANsignal() {
    ...
    tft.setTextDatum(BL_DATUM);
    if (SCANpause)
        tft.drawString("RSSI " + String(si4735.getCurrentRSSI()) + " dBuV ", 130 - d, 70);
    else
        tft.drawString("RSSI " + String((198 - ScanValueRSSI[int(currentScanLine)]) / signalScale, 0) + " dBuV ", 130 - d, 70);
    tft.setTextColor(TFT_ORANGE, TFT_GREY);
    if (SCANpause)
        tft.drawString(" SNR " + String(si4735.getCurrentSNR()) + " dB ", 130 - d, 80);
    else
        tft.drawString(" SNR " + String(ScanValueSNR[int(currentScanLine)]) + " dB ", 130 - d, 80);
}
...
void drawSCANline(int n) {
    ...
    if (n == int(currentScanLine)) {
        ...
        if (n == 0 or n == int(currentScanLine) + 1)
...
void DisplaySCAN() {
    ...
    if (setf) {
        ...
        if (posScan == posScanLast)
            ScanValueRSSI[posScan] = (ScanValueRSSI[posScan] + getSignal(true)) / 2;
        else
            ScanValueRSSI[posScan] = getSignal(true);
        if (posScan == posScanLast)
            ScanValueSNR[posScan] = (ScanValueSNR[posScan] + getSignal(false)) / 2;
        else
        ...
        if (ScanEmpty)
            setFreq(currentScanFreq + int((deltaScanLine - 159 + d + posScan + 1) * SCANstep));
        else
            freqUp();
    }
}
...
int getSignal(bool rssi) {
    ...
    if (rssi)
        res = 198 + (screenV * 40) - ((res / countScanSignal) * signalScale);
    else
        res /= countScanSignal;
    return (int)res;
}
...
void DrawSCANtxt(bool all) {
    ...
    if (posScan < 60 or all) {
        ...
        tft.setTextDatum(BL_DATUM);
        if (currentMode == FM)
            tft.drawString(String((currentScanFreq + (SCANstep * (deltaScanLine - 159 + (d / 2)))) / 100) + " MHz ", 0, 90);
        else
            tft.drawString(String(int(currentScanFreq + (SCANstep * (deltaScanLine - 159 + (d / 2))))) + " KHz ", 0, 90);
        // scale
    ...
    if (posScan > (240 - d) or all) {
        ...
        tft.setTextDatum(BR_DATUM);
        if (currentMode == FM)
            tft.drawString(" " + String((currentScanFreq + (SCANstep * (160 - (d / 2) + deltaScanLine))) / 100) + " MHz", 319 - d, 90);
        else
            tft.drawString(" " + String(int(currentScanFreq + (SCANstep * (160 - (d / 2) + deltaScanLine)))) + " KHz", 319 - d, 90);
        // scale
        tft.fillRect(296 - d, 181 + (d / 2), 23, 17, TFT_BLACK);
        tft.setTextColor(TFT_YELLOW, TFT_BLACK);
        if (SCANstep >= 1)
            tft.drawString("1:" + String(int(SCANstep)), 319 - d, 195 + (d / 2));
        else
            tft.drawString("x" + String(int(1 / SCANstep)), 319 - d, 195 + (d / 2));
    }
}
...
void DrawSCANind() {
    ...
    if (currentMode == FM)
        tft.drawString(String((band[bandIdx].currentStep) * 10) + " KHz", d + 25, 75);
    else
        tft.drawString(String(band[bandIdx].currentStep) + " KHz", d + 25, 75);
    // MODE
...
void initRetro() {
    ...
    if (bandRetro[RETROband].currentFreq < bandRetro[RETROband].minimumFreq or bandRetro[RETROband].currentFreq > bandRetro[RETROband].maximumFreq)
        bandRetro[RETROband].currentFreq = bandRetro[RETROband].minimumFreq;
...
void drawRETRO() {
    ...
    if (langRetroEN)
        tftRusPrint(String(bandRetro[RETROband].BandName), x, y);
    else
        tftRusPrint(String(bandRetro[RETROband].BandNameRU), x, y);
    // FM stereo
    tft.setTextSize(1);
    tft.setTextDatum(BL_DATUM);
    ...
    if (RETROband > 1)
        tft.drawString("KHz", x, y);
    else
        tft.drawString("MHz", x, y);
    //----------------------------------------------------------------------
    // draw grid retro -----------------------------------------------------
    for (int i = 0; i < 6; i++) {
...
void drawRETROscale() {
    ...
    if (RETROband == 0)
        Segment(String(Displayfreq, 1), "188.8", 0);
    else if (RETROband == 1)
        Segment(String(Displayfreq, 2), "88.88", 0);
    else if (RETROband == 2)
        Segment(String(Displayfreq, 0), "888", 0);
    else if (RETROband == 3)
        Segment(String(Displayfreq, 0), "1888", 0);
    else
        Segment(String(Displayfreq, 0), "88888", 0);
    //----------------------------------------------------------------------
...
void drawRetroBandBut() {
    ...
    for (int n = 0; n <= lastBandRetro; n++) {
        ...
        if (n == RETROband) {
            ...
            if (VHFon or n != 1)
                spr.pushImage(0, 0, But_Width, But_Height, (uint16_t *)But_retro);
            else
                spr.pushImage(0, 0, But_Width, But_Height, (uint16_t *)But_block);
        }
        if (screenV)
            spr.pushSprite(bandRetro[n].xPosV, bandRetro[n].yPosV);
        else
            spr.pushSprite(bandRetro[n].xPosH, bandRetro[n].yPosH);
        tftRusSetFont(T1516_T);
        tftRusSetStyle(NBL_T);
        tftRusSetDatum(BC_T);
        if (n == RETROband) {
            ...
            if (langRetroEN) {
                if (screenV)
                    tftRusPrint(String(bandRetro[n].BandName), bandRetro[n].xPosV + (But_Width / 2) + 3, bandRetro[n].yPosV + (But_Height / 2) + 9);
                else
                    tftRusPrint(String(bandRetro[n].BandName), bandRetro[n].xPosH + (But_Width / 2) + 3, bandRetro[n].yPosH + (But_Height / 2) + 9);
            } else {
                if (screenV)
                    tftRusPrint(String(bandRetro[n].BandNameRU), bandRetro[n].xPosV + (But_Width / 2) + 3, bandRetro[n].yPosV + (But_Height / 2) + 9);
                else
                    tftRusPrint(String(bandRetro[n].BandNameRU), bandRetro[n].xPosH + (But_Width / 2) + 3, bandRetro[n].yPosH + (But_Height / 2) + 9);
            }
            tftRusSetColor(TFT_OLIVE, TFT_TRANS);
        }
        if (langRetroEN) {
            if (screenV)
                tftRusPrint(String(bandRetro[n].BandName), bandRetro[n].xPosV + (But_Width / 2) + 2, bandRetro[n].yPosV + (But_Height / 2) + 8);
            else
                tftRusPrint(String(bandRetro[n].BandName), bandRetro[n].xPosH + (But_Width / 2) + 2, bandRetro[n].yPosH + (But_Height / 2) + 8);
        } else {
            if (screenV)
                tftRusPrint(String(bandRetro[n].BandNameRU), bandRetro[n].xPosV + (But_Width / 2) + 2, bandRetro[n].yPosV + (But_Height / 2) + 8);
            else
                tftRusPrint(String(bandRetro[n].BandNameRU), bandRetro[n].xPosH + (But_Width / 2) + 2, bandRetro[n].yPosH + (But_Height / 2) + 8);
        }
        if ((langRetroEN and bandRetro[n].RetroBandTime != "") or (!langRetroEN and bandRetro[n].RetroBandTimeRU != "")) {
            if (screenV)
                tft.drawRect(bandRetro[n].xPosV, bandRetro[n].yPosV, 220, 40, TFT_OLIVE);
            else
                tft.drawRect(bandRetro[n].xPosH, bandRetro[n].yPosH, 220, 40, TFT_OLIVE);
        }
        tftRusSetFont(T1012_T);
        tftRusSetStyle(NRG_T);
        tftRusSetDatum(BL_T);
        ...
        if (langRetroEN) {
            if (screenV)
                tftRusPrint(String(bandRetro[n].RetroBandTime), bandRetro[n].xPosV + 90, bandRetro[n].yPosV + 27);
            else
                tftRusPrint(String(bandRetro[n].RetroBandTime), bandRetro[n].xPosH + 90, bandRetro[n].yPosH + 27);
        } else {
            if (screenV)
                tftRusPrint(String(bandRetro[n].RetroBandTimeRU), bandRetro[n].xPosV + 90, bandRetro[n].yPosV + 27);
            else
                tftRusPrint(String(bandRetro[n].RetroBandTimeRU), bandRetro[n].xPosH + 90, bandRetro[n].yPosH + 27);
        }
    }
    spr.deleteSprite();
}
...
void displMEMO() {
    ...
    tft.setTextColor(TFT_YELLOW, TFT_GREY);
    if (presetBank)
        tft.drawString("PRESETS", XFreqDispl + d + 120, YFreqDispl + 78);
    else
        tft.drawString("       ", XFreqDispl + d + 120, YFreqDispl + 78);
    tft.setTextSize(1);
    tftRusSetSize(1);
    tftRusSetStyle(NBL_T);
    for (int i = -1; i < (2 + screenV); i++) {
        if (MEMOdel) {
            ...
            if (i) {
                if (presetBank)
                    tft.fillRect(d, (i * 40) + 120, 240, 40, TFT_NAVY);
                else
                    tft.fillRect(d, (i * 40) + 120, 240, 40, TFT_BLACK);
            } else
                tft.fillRect(d, 120, 240, 40, TFT_DARKCYAN);
        }
        if (presetBank) {
            if ((currentMemo + i) >= 0 and (currentMemo + i) <= lastPreset) {
                ...
                tftRusSetDatum(BR_T);
                tftRusSetColor(TFT_CYAN, TFT_TRANS);
                if (preset[currentMemo + i].presetIdx < 109)
                    tftRusPrint(String(preset[currentMemo + i].presetIdx, 2) + " MHz", d + 155, (i * 40) + 141);
                else
                    tftRusPrint(String(preset[currentMemo + i].presetIdx, 0) + " KHz", d + 155, (i * 40) + 141);
                // BAND
                if (i)
                    tft.setTextColor(2031, TFT_NAVY);
                else
                    tft.setTextColor(2031, TFT_DARKCYAN);
            ...
            if ((currentMemo + i) >= 0 and (currentMemo + i) <= lastMemoBank) {
                ...
                if ((MemoBank[currentMemo + i].freq < 153 or MemoBank[currentMemo + i].freq > 30000) and !(!i and MEMOadd)) {
                    ...
                    tftRusWidth = 12;
                    if (MEMOadd and !i)
                        tftRusPrint(String(addMemoName), d, 157);
                    else
                        tftRusPrint(String(MemoBank[currentMemo + i].Name), d, (i * 40) + 157);
                    tftRusSetCut(0, 0);
                    tftRusSetDatum(BR_T);
                    tftRusSetColor(TFT_CYAN, TFT_TRANS);
                    if (MEMOadd and !i) {
                        if (addMemoBand == FM)
                            tftRusPrint(String(float(addMemoFreq) / 100, 2) + " MHz", d + 155, 141);
                        else
                            tftRusPrint(String(addMemoFreq) + " KHz", d + 155, 141);
                    } else {
                        if (MemoBank[currentMemo + i].band == FM)
                            tftRusPrint(String(float(MemoBank[currentMemo + i].freq) / 100, 2) + " MHz", d + 155, (i * 40) + 141);
                        else
                            tftRusPrint(String(MemoBank[currentMemo + i].freq) + " KHz", d + 155, (i * 40) + 141);
                    }
                    // BAND
                    if (i)
                        tft.setTextColor(2031, TFT_BLACK);
                        ...
                        tft.setTextColor(2031, TFT_DARKCYAN);
                    if (MEMOadd and !i)
                        tft.drawString(band[addMemoBand].bandName, d + 175, 137);
                    else
                        tft.drawString(band[(MemoBank[currentMemo + i].band & 0x1F)].bandName, d + 175, (i * 40) + 137);
                    tft.drawRect(d + 155, (i * 40) + 124, 39, 16, 2031);
                    // MODE
                    if (i)
                        tft.setTextColor(TFT_YELLOW, TFT_BLACK);
                        ...
                        tft.setTextColor(TFT_YELLOW, TFT_DARKCYAN);
                    if (MEMOadd and !i)
                        tft.drawString(bandModeDesc[addMemoMode], d + 215, 137);
                    else
                        tft.drawString(bandModeDesc[int(MemoBank[currentMemo + i].band / 32)], d + 215, (i * 40) + 137);
                    tft.drawRect(d + 195, (i * 40) + 124, 39, 16, TFT_YELLOW);
                }
            }
        }
...
int bandFreq(float freq) {
    ...
    if (freq < 64 or freq > 108) {
        ...
        bool flag = false;
        while (n < 29 and !flag)
            if (freq >= band[n].minimumFreq and freq <= band[n].maximumFreq)
                flag = true;
            else
                n++;
    }
        ...
        case 2:
            switch (pos) {
                ...
                case 2:
                    if (prevsaverTime == 2)
                        prevsaverTime = 5;
                    else if (prevsaverTime == 5)
                        prevsaverTime = 10;
                    else if (prevsaverTime == 10)
                        prevsaverTime = 30;
                    else if (prevsaverTime == 30)
                        prevsaverTime = 2;
                    break;
                case 3:
...
void saveSETUP() {
    ...
    if (VHFon != prevVHFon or langRetroEN != prevlangRetroEN or beeperOn != prevbeeperOn or digitLigth != prevdigitLigth or loadMemory != prevloadMemory or
        ...
        screenV != prevscreenV or displayOff != prevdisplayOff or minSCANstep != prevminSCANstep or maxSCANstep != prevmaxSCANstep or autoSCANstep != prevautoSCANstep or
        SCANaccuracy != prevSCANaccuracy or displayPower != prevdisplayPower or RDSalways != prevRDSalways or seekAccuracy != prevseekAccuracy) {
...
int confirm(String text) {
    ...
    int d = !screenV * 40;
    if (!screenV)
        tftTransRect(0, 0, 320, 240, TFT_MAROON);
    else
        tftTransRect(0, 0, 240, 320, TFT_MAROON);
...
void presetLoad() {  // loading preset by city name -----------------------------
    ...
    for (int i = 0; i <= lastMemory; i++) {
        if (memory[i].memoryGroup == "" and presetBank) {
            ...
            preset[count].PresetName = (char *)memory[i].MemoryName;
            count++;
        } else {
            int n = strlen(memory[i].memoryGroup) + 1;
            char p[n];
            char *pp = (char *)memory[i].memoryGroup;
    ...
    if (memoPreset and !presetBank) {
        for (int i = 0; i <= lastMemoBank; i++) {
            if (MemoBank[i].freq >= 153 and MemoBank[i].freq <= 30000) {
                if (MemoBank[i].band)
                    preset[count].presetIdx = MemoBank[i].freq;
                else
                    preset[count].presetIdx = float(MemoBank[i].freq) / 100;
                preset[count].PresetName = MemoBank[i].Name;
                int j = 19;
                while (j > 0 and (MemoBank[i].Name[j] == char(32) or MemoBank[i].Name[j] == char(0))) j--;
                preset[count].PresetName[j + 1] = '\0';
                preset[count].presetPos = 0;
                count++;
            }
        }
                    ...
                    for (int yy = 0; yy < (8 - (yb * 4 * (1 - tftRusFont)) - (tftRusBottomCut * (yb * (2 + tftRusFont)))); yy++) {
                        ...
                        for (int sy = 0; sy < tftRusSize; sy++) {
                            for (int sx = 0; sx < tftRusSize; sx++) {
                                ...
                                if (bi) {
                                    if (tftRusColor >= 0)
                                        tft.drawPixel(x + (i * tftRusSize * tftRusWidth) + (xx * tftRusSize) + sx + cd, y + (yb * tftRusSize * 8) + (yy * tftRusSize) + sy,
                                                      tftRusColor);
                                } else {
                                    if (tftRusBack >= 0)
                                        tft.drawPixel(x + (i * tftRusSize * tftRusWidth) + (xx * tftRusSize) + sx + cd, y + (yb * tftRusSize * 8) + (yy * tftRusSize) + sy,
...
void drawBut(uint8_t id, uint8_t state, String alt) {
    ...
    if (state == B_BLOCK)
        ...
        spr.pushImage(0, 0, But_Width, But_Height, (uint16_t *)But_jam);
    else
        spr.pushImage(0, 0, But_Width, But_Height, (uint16_t *)But_select);
    if (state == B_BLOCK)
    ...
    if (state != B_JAM && state != B_SELECT) {
        if (state == B_BLOCK || type != B_GOLD)
            spr.setTextColor(TFT_YELLOW);
        else
            spr.setTextColor(TFT_OLIVE);
        spr.drawString(alt, (But_Width / 2) + 2, (But_Height / 2) + 9);
    }
    if (screenV)
        spr.pushSprite(but[id].xPosV, but[id].yPosV);
    else
        spr.pushSprite(but[id].xPosH, but[id].yPosH);
    spr.deleteSprite();
}
